    ...
    while true do
        ...
        if inTrash then
            ...
            if DoesEntityExist(dumpster) or not IsPedDeadOrDying(PlayerPedId()) or not IsPedFatallyInjured(PlayerPedId()) then
                ...
                DrawText3Ds(dumpPos.x, dumpPos.y, dumpPos.z + 1.1, 'Press [~y~E~w~] to get out!')
    ...
    while true do
        ...
        if canHide then
            ...
            for i = 1, #dumpsters do
                ...
                if dist < 1.8 then
                    if not inTrash then
                        DrawText3Ds(dumpPos.x, dumpPos.y, dumpPos.z + 1.1, 'Press [~y~E~w~] to get inside')
                        if IsControlJustReleased(0, 38) and not inTrash then
                            if DoesEntityExist(ped) then
                                ...
                                exports['mythic_notify']:DoHudText('error', 'Someone is already hiding in the dumpster!')
                                end
                            end
                        end
                    end
